...
!-----------------------------------------------------------------------
      ...
      real :: cloud_base,cloud_top
      real :: rhsfc,qtsfc,ql1,qt1,qt2,p1,tlr,qtlr
...
!!!        rh0(i,j,k)=1.0
        ! linearly decrease from surface to cloud base:
        if( zh(i,j,k).lt.cloud_base )then
          rh0(i,j,k) = rh_sfc + (1.0-rh_sfc)*(zh(i,j,k)-0.0)/(cloud_base-0.0)
        else
          rh0(i,j,k) = 1.0
        endif
      enddo
...
!!!            qc0(i,j,k)=qcval
!!!            qc0(i,j,k)=qtval-qv0(i,j,k)
            if( zh(i,j,k).ge.cloud_base .and. zh(i,j,k).le.cloud_top )then
              qc0(i,j,k) = 0.0001*(zh(i,j,k)-cloud_base)/(cloud_top-cloud_base)
            else
              qc0(i,j,k) = 0.0
            endif
            qtp=qv0(i,j,k)+qc0(i,j,k)
...
!!!            if(n.gt.180.and.dowr) write(outfile,*) n,tp,tm,delz
            if(n.gt.180.and.dowr) write(outfile,*) n,prs0(i,j,k),thlast,thn,thn-thlast
            if(abs(thn-thlast).gt.0.0005 .and. n.lt.200)then
              thn=thlast+0.3*(thn-thlast)
              thlast=thn
              go to 572
            elseif(n.ge.200)then
              if(dowr) write(outfile,*) '  stuck in loop (333)!'
              print *,'  prs,thlast,thn = ',prs0(i,j,k),thlast,thn
              call stopcm1
            endif
...
!------------------------------------------------------------------
      ...
      elseif( isnd.eq.22 )then
        ...
        ! 180815:  New code, constant vertical gradients of temperature 
        !          and total-water mixing ratio. 

        tsfc   =     288.0    ! temperature at surface (K)
        psfc   =  100000.0    ! pressure at surface (Pa)
        rhsfc  =      0.80    ! relative humidity at surface (nondimensional)
        tlr    =    var1      ! temperature lapse rate (K/m)
        qtlr   =    var2      ! total water lapse rate (g/g/m)
        ...
        do i=1,ni
          ...
          pi1 = (psfc*rp00)**(rd/cp)
          pi2 = pi1
          qtsfc = rhsfc*rslf(psfc,tsfc)
          qv2 = qtsfc
          qt1 = qv2
          qt2 = qv2
          thv1 = (tsfc/pi1)*(1.0+reps*qtsfc)/(1.0+qtsfc)
          ! k loop:
          do k=1,nk
            ! new values of T and qt:
            t2 = tsfc + zh(i,j,k)*tlr
            qt2 = qtsfc + zh(i,j,k)*qtlr
            print *,'zh,t2,qt2 = ',zh(i,j,k),t2,qt2
            thvlast = 0.0
            thv2 = 1000.0
            ! iteration loop:
            n = 0
            do while( abs(thv2-thvlast).ge.0.0001 )
              thvlast = thv2
              n = n+1
              thv2 = (t2/pi2)*(1.0+reps*qv2)/(1.0+qt2)
              ! this is the hydrostatic equation:
              pi2 = pi1-g*(zh(i,j,k)-z1)/(cp*0.5*(thv2+thv1))
              p2 = pi2**(cp/rd) * p00
              qvs = rslf(p2,t2)
              ...
              print *,'  p2,ql2,rh = ',n,p2,ql2,qv2/qvs
              if( n.gt.100 )then
                print *
                print *,'  iterative loop is not converging '
                print *
                stop 15151
              endif
            enddo
            !  Done.  Save results, prepare for next level.
            t0(i,j,k) = t2
            thv0(i,j,k) = thv2
            pi0(i,j,k) = pi2
            prs0(i,j,k) = p2
            ...
            qc0(i,j,k) = ql2
            th0(i,j,k) = t0(i,j,k)/pi0(i,j,k)
            rh0(i,j,k) = qv0(i,j,k)/rslf(prs0(i,j,k),t0(i,j,k))
            z1 = zh(i,j,k)
            t1 = t2
            qt1 = qt2
            thv1 = thv2
