...
!-----------------------------------------------------------------------------
      ...
      real, intent(in), dimension(itb:ite,jtb:jte) :: rgz,gzu
...
!-----------------------------------------------------------------------
      ...
      integer :: tctype
      real :: dr,tem
      double precision :: tem1,tem2,temd,dtem1,dtem2
      logical :: doit
...
!---- This is here to ensure that certain idealized cases work ----
      ...
      IF( (isnd.eq.4 .or. isnd.eq.9 .or. isnd.eq.10 .or. isnd.eq.11 .or. isnd.eq.15 .or. isnd.eq.22) )THEN
...
!!!          if(dowr) write(outfile,*) n,pdata(n,prx),pdata(n,pry),pdata(n,prz)
        ...
        IF( terrain_flag )THEN
          ! 180715:  get terrain height and sigma
          if( prsig.ge.1 )then
...
!  reference:
      ...
      IF(iinit.eq.1)THEN
        ...
        zc      =   1900.0  ! height of center of bubble above ground (m)
        bhrad   =   2500.0  ! horizontal radius of bubble (m)
        bvrad   =   1900.0  ! vertical radius of bubble (m)
        bptpert =      0.02  ! max potential temp perturbation (K)
...
!!!          endif
          ...
          if( abs(zh(i,j,k)-zc).le.bvrad )then
            ! potential temperature perturbation:
            tha(i,j,k)=bptpert*cos(0.5*pi*(xh(i)-ric)/bhrad)  &
                              *cos(0.5*pi*(zh(i,j,k)-zc)/bvrad)
            ! dum1 is streamfunction:
            dum1(i,j,k)=cos(0.5*pi*(xf(i)-(ric+0.25*(maxx-minx)))/bhrad)  &
                       *cos(0.5*pi*(zf(i,j,k)-zc)/bvrad)  &
                       * 4.76e-4 * 1000.0 * 1000.0
          endif
        ...
        ! Get u and v from streamfunction:
        do k=1,nk+1
        do j=1,nj
        do i=1,ni+1
          ua(i,j,k) = -(dum1(i,j,k+1)-dum1(i,j,k))/(zf(i,j,k+1)-zf(i,j,k))
          wa(i,j,k) =  (dum1(i+1,j,k)-dum1(i,j,k))/(xf(i+1)-xf(i))
        enddo
        enddo
        enddo
      ...
      IF( tctype.eq.1 )THEN
        ...
        do i=1,nref
          if(rref(i).lt.r0)then
            ...
            vt(i) = sqrt( vmax**2 * (rref(i)/rmax)**2     &
...
!$omp private(i,j,k)
        ...
        do i=ib,ie
          ...
          uten(i,j,k)=0.0
          vten(i,j,k)=0.0
          rru(i,j,k) = ua(i,j,k)*rho0(1,1,k)
          rrv(i,j,k) = 0.0
          rrw(i,j,k) = wa(i,j,k)*rf0(1,1,k)
        enddo
        enddo
        enddo
        ...
        ! 180815:  advection terms
          call advu( 3    ,arh1,arh2,uh,xf,rxf,arf1,arf2,uf,vh,gz,rgz,gzu,mh,rho0,rr0,rf0,rrf0,dum1,dum2,dum3,dum4,dum5,dum6,dum7,divx, &
                     rru,ua ,uten,rrv,rrw,rdsf,c1,c2,rho,dtl  ,.false.,uten1,wprof)
          call   advw( 3    ,xh,rxh,arh1,arh2,uh,xf,vh,gz,rgz,mh,mf,rho0,rr0,rf0,rrf0,  &
                      dum1,dum2,dum3,dum4,dum5,dum6,dum7,divx,                       &
                      rru,rrv,rrw,wa   ,wten,rds,rdsf,c1,c2,rho,dtl  ,               &
                      .false.,wten1,hadvordrv,vadvordrv,advwenov)
...
!!!          call satadj(4,dtl,dtem1,dtem2,ruh,rvh,rmh,pi0,th0,   &
!!!                      rho,dum3,ppi,prs,tha,qa)
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine getset(restarted,mass1,ruh,rvh,xh,yh,xf,yf,               &
      ...
      real, intent(in), dimension(ib:ie) :: ruh,xh
      real, intent(in), dimension(jb:je) :: rvh,yh
